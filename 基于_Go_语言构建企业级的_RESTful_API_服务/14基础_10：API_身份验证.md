...
**JWT 认证流程**
![](https://user-gold-cdn.xitu.io/2018/6/18/164107dc88e79a61?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
1. 客户端使用用户名和密码请求登录
...
![](https://user-gold-cdn.xitu.io/2018/6/2/163bfc8ea855234a?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
最后生成的 Token 像这样：
...
> 详细的 JWT 介绍参考 [JWT – 基于 Token 的身份验证](https://link.juejin.im?target=https%3A%2F%2Fblog.csdn.net%2Fqq_28098067%2Farticle%2Fdetails%2F52036493)。
...
在 `handler/user/login.go`（详见 [demo09/handler/user/login.go](https://link.juejin.im?target=https%3A%2F%2Fgithub.com%2Flexkong%2Fapiserver_demos%2Fblob%2Fmaster%2Fdemo09%2Fhandler%2Fuser%2Flogin.go)）中添加 login 的具体实现：

1. 解析用户名和密码
2. 通过 `auth.Compare()` 对比密码是否是数据库保存的密码，如果不是，返回 `errno.ErrPasswordIncorrect` 错误
3. 如果相同，授权通过，通过 `token.Sign()` 签发 token 并返回

> `auth.Compare()` 的实现详见 [demo09/pkg/auth/auth.go](https://link.juejin.im?target=https%3A%2F%2Fgithub.com%2Flexkong%2Fapiserver_demos%2Fblob%2Fmaster%2Fdemo09%2Fpkg%2Fauth%2Fauth.go)。
> 
> `token.Sign()` 的实现详见 [demo09/pkg/token/token.go](https://link.juejin.im?target=https%3A%2F%2Fgithub.com%2Flexkong%2Fapiserver_demos%2Fblob%2Fmaster%2Fdemo09%2Fpkg%2Ftoken%2Ftoken.go)。

**API 添加认证 middleware**

在 `router/router.go` 中对 user handler 添加授权 middleware：
![](https://user-gold-cdn.xitu.io/2018/6/2/163bfbb7b2c4758a?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
> `middleware.AuthMiddleware()` 实现详见 [demo09/router/middleware/auth.go](https://link.juejin.im?target=https%3A%2F%2Fgithub.com%2Flexkong%2Fapiserver_demos%2Fblob%2Fmaster%2Fdemo09%2Frouter%2Fmiddleware%2Fauth.go)。
> 
> `token.ParseRequest()` 实现详见 [demo09/pkg/token/token.go](https://link.juejin.im?target=https%3A%2F%2Fgithub.com%2Flexkong%2Fapiserver_demos%2Fblob%2Fmaster%2Fdemo09%2Fpkg%2Ftoken%2Ftoken.go)。
...
1. 将 `apiserver_demos/demo09` 复制为`$GOPATH/src/apiserver`
